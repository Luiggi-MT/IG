...
_vertex3f _cylinder::obtener_punto(_vertex3f punto, float radian) {
    float radio = punto.x;
    return _vertex3f(radio * cos(radian), punto.y, -radio * sin(radian));
}

// Generar los vértices del cono por barrido
void _cylinder::generar_por_barrido(vector<_vertex3f> curva_generatriz) {
...
void _cylinder::generar_triangulos(unsigned int puntos)
{
